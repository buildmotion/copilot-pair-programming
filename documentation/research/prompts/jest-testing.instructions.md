...
describe('ErrorHandlerService', () => {
  ...
  it('should handle client errors correctly', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('Client Error: Test error', error);
  });
  ...
  it('should not log to external service when not configured', () => {
    ...
    expect(console.log).not.toHaveBeenCalledWith(expect.stringContaining('Logging to external service'), expect.anything());
  });
});
```
...
## Automated Jest Spec Test Workflow for Angular File Changes

**Copilot Directives:**

1. When any Angular file matching `**/*.component.ts` or `**/*.service.ts` is created or modified, automatically prompt:

> "Align the corresponding spec tests for code coverage and alternate/negative flow tests. Ensure all public methods and critical logic are covered with happy path, edge case, and error scenario tests. Update or create the spec file to follow jest-testing best practices."

2. To manually trigger automated spec test creation, update, or coverage verification, use:

> `cp jest <file name>`

This command will:

- Create or update the corresponding spec file for the given Angular service/component
- Ensure tests cover all public methods, edge cases, and error scenarios
- Verify code coverage and reliability
- Prompt for missing or incomplete tests

**Example:**

> `cp jest libs/agency/supabase/src/lib/services/supabase.service.ts`

This will automate the process of generating, updating, and validating the spec file for the specified service/component, ensuring best practices and coverage requirements are met.
...
describe('ErrorHandlerService', () => {
  ...
  it('should handle client errors correctly', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('Client Error: Test client error', error);
  });
  ...
  it('should handle HTTP errors correctly', () => {
    ...
    expect(console.error).toHaveBeenCalledWith(`HTTP Error: ${httpError.message}`, httpError);
  });
});
```
...
describe('ErrorHandlerService', () => {
  ...
  it('should not log to external service when not configured', () => {
    ...
    const serviceWithoutExternalLogging = new ErrorHandlerService(configWithoutExternalLogging);
  ...
  it('should respect includeDefaultErrorHandling flag', () => {
    ...
    try {
      const serviceWithoutDefaultHandling = new ErrorHandlerService(configWithoutDefaultHandling);
...
describe('ErrorHandlerService', () => {
  ...
  it('should log initialization with config', () => {
    expect(console.info).toHaveBeenCalledWith('ErrorHandlerService initialized with config:', mockConfig);
  });
  ...
  it('should log to external service when configured', () => {
    ...
    expect(console.log).toHaveBeenCalledWith(`Logging to external service: ${mockConfig.externalLoggingConfig?.endpoint}`, {
...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [AuthService, { provide: AUTH_CONFIG, useValue: mockAuthConfig }],
    });
...
## Service Spec Test Efficiency: Actionable Improvements (2025-07-21)

### 1. Explicit Mocking Patterns for Third-Party Clients

Use clear recipes for mocking nested properties and methods to avoid TypeScript and runtime errors:
...
const mockSupabase = { auth: mockAuth } as any;
jest.mock('@supabase/supabase-js', () => ({
  createClient: jest.fn().mockReturnValue(mockSupabase),
}));
```

### 2. Alias and Import Troubleshooting

Always use the correct npm scope for internal libraries (e.g., `@buildmotion-ai/github-user`).
If you get "Cannot find module" errors, check your `tsconfig.base.json` and ensure the alias maps to the package's `src/index.ts`.

### 3. Service Spec Test Template

Standardize your service test structure:

```typescript
import { MyService } from '@myorg/my-service';
describe('MyService', () => {
  ...
  // Add error/edge case tests here
});
```

### 4. Common Error Patterns and Fixes

Quick solutions for common Jest/TypeScript errors:

- "Type 'unknown' is not assignable": Use `as any` for deep mocks, or define the full mock type.
- "Cannot find module": Check import alias and tsconfig paths.
- "Mock not called": Ensure the mock is passed to the service, not the real implementation.

### 5. Test Update Checklist

- [ ] Update/verify import paths and aliases
- [ ] Mock all external dependencies before instantiating the service
- [ ] Use `jest.clearAllMocks()` in `beforeEach`
- [ ] Cover happy path, error, and edge cases
- [ ] Run tests after every change and fix errors immediately
